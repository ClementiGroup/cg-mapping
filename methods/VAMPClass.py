...
class VAMP():
    ...
    def __init__(
        self, kirchoff_matrix : np.ndarray = None, kbT : int = 1, n_beads : int = 2, lagtimes : List = None, dt : float = 0.005
    ):
        '''
        '''
    ...
    def compute_vamp(self,lagtime):
        ...
        for mapping_matrix in mapping_matrices:
            ...
            C0tau = Qn@mapping_matrix@self.inv_kirchoff_matrix@omega_tau@mapping_matrix.T@Qn
    ...
    def compute_omega_tau(self):
        ...
        for (eigval,eigvec) in zip(self.eigvals,self.eigvecs):
            omega_tau += np.outer(eigvec,eigvec)*np.exp(-eigval*self._lagtime*self.dt)
        return omega_tau
